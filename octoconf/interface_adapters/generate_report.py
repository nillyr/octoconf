...
class ReportGeneratorInterfaceAdapter(IReportGenerator):
    ...
    def _add_information_worksheet(self, baseline: str) -> None:
        ...
        # Value
        ws.write("D14", baseline, self._get_format("regular"))
        # Key
    ...
    def generate_report(self, data: list, baseline: str) -> str:
        self._add_information_worksheet(baseline)
        self._add_synthesis_worksheet(data["categories"])
        self._write_results(data["categories"])
        self.wb.close()
        self.wb = None
